...
test = {"cylinders":d3.scaleOrdinal(d3.schemeCategory10)}
...
var colorMapping = {"cylinders":d3.scaleOrdinal(d3.schemeCategory10),
                    "economy (mpg)":d3.scaleSequential(d3.interpolateBlues),
                    "displacement (cc)":d3.scaleSequential(d3.interpolateOranges),
                    "power (hp)":d3.scaleSequential(d3.interpolateGreens),
                    "weight (lb)":d3.scaleSequential(d3.interpolateReds),
                    "0-60 mph (s)":d3.scaleSequential(d3.interpolateGreys),
                    "year":d3.scaleSequential(d3.interpolatePurples)
                }

// Map for referencing min/max per each attribute
var extentByAttribute = {};
var allAttributeExtent = {};
...
d3.csv('cars.csv', dataPreprocessor).then(function(dataset) {
        ...
        for (var key in dataset[0]){
            if (attribute !== "name")
                allAttributeExtent[key] = d3.extent(dataset, d => d[key]);
        }

        for (var attribute in allAttributeExtent){
            if (attribute !== "name" && attribute !== "cylinders")
                colorMapping[attribute].domain(allAttributeExtent[attribute]);
        }
...
function onNameChanged() {
    ...
    selectColorMap = select.options[select.selectedIndex].value
    d3.selectAll(".dot").style("fill", function(d){ return colorMapping[selectColorMap](d[selectColorMap]); });
}
